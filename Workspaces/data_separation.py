...
def remove_nans_feature(feature_list, feature_data_path, feature_files):
    ...
    for key in feature_data_filtered:
        feature_data_removed_nans[key] = feature_data_filtered[key][~feature_data_filtered[key][list(set(feature_list) - set(['Deff2', 'Mean Deff2']))].isin([np.nan, np.inf, -np.inf]).any(axis=1)]
