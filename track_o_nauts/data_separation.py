...
def read_feature(feature_data_path, feature_files):
    ...
    for item in feature_files:
        ...
        file_name = os.path.splitext(item)[0]  # rename
...
def remove_nans_feature(feature_list, feature_data_path, feature_files):
    ...
    for key in feature_data_filtered:
        ...
        feature_data_removed_nans[key] = feature_data_filtered[key][~feature_data_filtered[key][list(
                                        set(feature_list) - set(['Deff2', 'Mean Deff2'])
                                        )].isin([np.nan, np.inf, -np.inf]).any(axis=1)]
...
def put_together(json_data, feature_list, feature_data_path, feature_files):
    ...
    for key in feature_data_removed_nans:
        ...
        feature_data_removed_nans[key]['Category'] = pd.cut(
                                                    feature_data_removed_nans[key]['Quality'], bins=3, labels=category_labels
                                                    )
